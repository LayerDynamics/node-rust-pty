...
// Import the native binding correctly using namespace import
import * as nativeBinding from "../darwin-x64/index.js";
...
async function createPtyHandle(t) {
	try {
		...
		const ptyHandlePromise = PtyHandle.new(); // Use 'new' as a factory method
...
test("operations after close", async (t) => {
	...
	try {
		...
		const closedPtyHandle = await PtyHandle.new(); // Directly using PtyHandle.new()
		if (!closedPtyHandle) {
...
test("cleanup on drop", async (t) => {
	...
	let ptyHandle = await PtyHandle.new(); // Directly using PtyHandle.new()
	if (!ptyHandle) {
	...
	try {
		const newPtyHandle = await PtyHandle.new(); // Directly using PtyHandle.new()
		if (!newPtyHandle) {
