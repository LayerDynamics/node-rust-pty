...
import { JSDOM } from "jsdom"; // Added JSDOM to simulate a browser environment

let ptyHandle;
let terminal;
let jsdom; // JSDOM instance

// Run before each test case
test.beforeEach(() => {
	// Initialize jsdom to simulate the DOM environment
	jsdom = new JSDOM(`<!DOCTYPE html><div id="terminal-container"></div>`);
	const { window } = jsdom;

	// Set global `document` and `self` to simulate the browser environment
...
test.afterEach(async () => {
	...
	jsdom.window.close(); // Close the JSDOM instance
});
