...
test('multiple reads', async t => {
  const ptyHandle = await PtyHandle.new();
  const readPromises = Array(5).fill().map(() => ptyHandle.read());
...
test('write and read', async t => {
  ...
  const readResult = await ptyHandle.read();
  t.true(readResult.includes(testString), 'Read result should include the written string');
});
...
test('resize and read', async t => {
  ...
  const readResult = await ptyHandle.read();
  t.is(typeof readResult, 'string', 'Read result after resize should be a string');
});
