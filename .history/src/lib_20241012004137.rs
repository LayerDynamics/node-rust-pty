...
// Define a type alias for napi::Error with Status as the generic parameter
type NapiError = napi::Error<Status>;
...
impl PtyHandle {
    ...
    /// ```
    #[napi(constructor)]
    pub fn new(shell: String, args: Vec<String>) -> Result<Self, NapiError> {
            ...
            let master_fd_opt = {
                ...
                pty_guard
                    ...
                    .ok_or_else(|| {
                        ...
                        NapiError::new(Status::GenericFailure, "PTY not initialized")
                    })
            }?;
    ...
    /// ```
    #[napi]
    pub async fn write(&self, data: String) -> Result<(), NapiError> {
        let pty_lock = Arc::clone(&self.pty);
        ...
        let write_future = task::spawn_blocking(move || -> Result<(), NapiError> {
            let master_fd_opt = {
                let pty_guard = pty_lock.lock().map_err(|e| {
                    error!("Failed to acquire PTY lock: {}", e);
                    NapiError::new(Status::GenericFailure, "Failed to acquire PTY lock")
    ...
    /// ```
    #[napi]
    pub async fn resize(&self, cols: u16, rows: u16) -> Result<(), NapiError> {
        let pty_lock = Arc::clone(&self.pty);
        ...
        let resize_future = task::spawn_blocking(move || -> Result<(), NapiError> {
        ...
        match timeout(resize_timeout, resize_future).await {
            Ok(result) => match result {
                Ok(_) => Ok(()),
                Err(e) => Err(e),
            },
    ...
    /// ```
    #[napi]
    pub async fn close(&self) -> Result<(), NapiError> {
        let pty_lock = Arc::clone(&self.pty);
        ...
        let close_future = task::spawn_blocking(move || -> Result<(), NapiError> {
        ...
        match timeout(close_timeout, close_future).await {
            Ok(result) => match result {
                Ok(_) => Ok(()),
                Err(e) => Err(e),
            },
