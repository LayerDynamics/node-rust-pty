...
impl PtyHandle {
    ...
    /// Reads data from the master PTY asynchronously
    ///
    /// This function spawns a blocking task to read from the master PTY,
    /// returning the output as a string.
    ///
    /// # Returns
    /// A `Result` containing the output string or an error if the operation fails.
    ///
    /// # Examples
    /// ```javascript
    ...
    pub async fn read(&self) -> Result<String> {
        ...
        let data = task::spawn_blocking(move || -> Result<String> {
            let master_fd_opt = {
        ...
        Ok::<String, napi::Error>(data)
    }

    /// Writes data to the master PTY asynchronously
    ///
    /// This function writes the provided string to the master PTY.
    ///
    /// # Parameters
    ...
    /// Resizes the PTY terminal window asynchronously
    ///
    /// This method changes the terminal's dimensions (rows and columns).
    ///
    /// # Parameters
    ...
    pub async fn resize(&self, cols: u16, rows: u16) -> Result<()> {
        ...
        .await
        .map_err(|e| {
            error!("Resize task failed: {:?}", e);
            Error::new(
                Status::GenericFailure,
                format!("Resize task failed: {:?}", e),
            )
        })??;

        Ok::<(), napi::Error>(resize_result)
    }

    /// Closes the PTY session asynchronously
    ///
    ...
    pub async fn close(&self) -> Result<()> {
        ...
        let close_result = task::spawn_blocking(move || -> Result<()> {
            ...
            if let Some(pty) = pty_option.take() {
                ...
                if kill_ret != 0 {
                    ...
                    // This is a simplistic approach; in a real-world scenario, you might want to implement a more robust waiting mechanism
        ...
        .await
        .map_err(|e| {
            error!("Close task failed: {:?}", e);
            Error::new(
                Status::GenericFailure,
                format!("Close task failed: {:?}", e),
            )
        })??;

        Ok::<(), napi::Error>(close_result)
    }
}
