...
impl PtyHandle {
    ...
    pub async fn new() -> Result<Self> {
        ...
        // Spawn a blocking task to initialize PtyProcess and set up channels
        let pty_handle = timeout(creation_timeout, task::spawn_blocking(|| {
        ...
        .await
        .map_err(|e| Error::new(Status::GenericFailure, format!("PtyHandle creation timed out: {:?}", e)))?
        .map_err(|e| {
            Error::new(Status::GenericFailure, format!("Failed to create PtyHandle: {:?}", e))
        })?;

        // Now pty_handle is of type PtyHandle, safe to call methods on it
