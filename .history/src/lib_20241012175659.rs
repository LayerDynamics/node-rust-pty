...
impl PtyHandle {
    ...
    #[napi(static_method)]
    pub async fn new_handle() -> Result<Self> {
        initialize_logging();
        ...
        let close_result = timeout(graceful_timeout, task::spawn_blocking(move || -> std::result::Result<(), String> {
            ...
            if let Some(pty) = pty_option.take() {
                ...
                if let Err(e) = pty.kill_process(libc::SIGTERM) {
                    ...
                    pty.waitpid(libc::WNOHANG).map(|_| ()).map_err(|e| e.to_string())?;
    ...
    /// Listens for results and handles them asynchronously.
...
pub async fn test_pty_handle_creation() -> Result<bool> {
    ...
    let pty_handle = PtyHandle::new_handle().await?;
