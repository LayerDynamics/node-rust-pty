...
impl PtyHandle {
    // Simplified to handle a single instance or sequential operations
...
impl PtyHandle {
    ...
    pub async fn new() -> Result<Self> {
        ...
        // Creating a new PtyHandle should be a rare operation, ensure that it only happens when necessary
        ...
        // Start handling commands and listening for results
...
pub async fn read(&self) -> Result<String> {
    ...
    // Spawn the blocking task and set a timeout
    ...
    // Match on the outer timeout and inner result handling
    match result {
        // Unwrap the nested `Result` with `?`
        Ok(Ok(data)) => {
            self.send_result(PtyResult::Success("Read operation successful".into()));
            Ok(data)
        }
        // Convert the JoinError to a napi::Error
        Ok(Err(join_err)) => {
            error!("Error during task join: {:?}", join_err);
            Err(Error::new(Status::GenericFailure, format!("Task join error: {:?}", join_err)))
        }
