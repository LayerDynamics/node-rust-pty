...
impl PtyHandle {
    ...
    pub async fn read(&self) -> Result<String> {
        ...
        let read_future = task::spawn_blocking(move || -> Result<String> {
            let master_fd_opt = {
        ...
        // Await the read operation with timeout and handle errors appropriately
        let read_result = timeout(read_timeout, read_future).await.map_err(|_| {
            Error::new(
                Status::GenericFailure,
        ...
        let data = read_result?; // Propagate napi::Error
    ...
    pub async fn write(&self, data: String) -> Result<()> {
        ...
        let write_future = task::spawn_blocking(move || -> Result<()> {
            let master_fd_opt = {
                let pty_guard = pty_lock.lock().map_err(|e| {
                    error!("Failed to acquire PTY lock: {}", e);
                    Error::new(Status::GenericFailure, "Failed to acquire PTY lock")
        ...
        // Await the write operation with timeout and handle errors appropriately
        let write_result = timeout(write_timeout, write_future).await.map_err(|_| {
            Error::new(
                Status::GenericFailure,
        ...
        write_result?; // Propagate napi::Error
    ...
    pub async fn resize(&self, cols: u16, rows: u16) -> Result<()> {
        ...
        let resize_future = task::spawn_blocking(move || -> Result<()> {
        ...
        // Await the resize operation with timeout and handle errors appropriately
        let resize_result = timeout(resize_timeout, resize_future).await.map_err(|_| {
            Error::new(
                Status::GenericFailure,
        ...
        resize_result?; // Propagate napi::Error
    ...
    pub async fn close(&self) -> Result<()> {
        ...
        let close_future = task::spawn_blocking(move || -> Result<()> {
        ...
        // Await the close operation with timeout and handle errors appropriately
        let close_result = timeout(close_timeout, close_future).await.map_err(|_| {
            Error::new(
                Status::GenericFailure,
        ...
        close_result?; // Propagate napi::Error
        ...
        task::block_in_place(|| {
            ...
            if let Some(pty) = pty_option.take() {
                ...
                if unsafe { libc::close(pty.master_fd) } != 0 {
                    ...
                    info!(
                        "Dropping: Master_fd {} closed successfully.",
                        pty.master_fd
                    );
                }
                ...
                if unsafe { libc::close(pty.slave_fd) } != 0 {
                    ...
                    info!(
                        "Dropping: Slave_fd {} closed successfully.",
                        pty.slave_fd
                    );
                }

                info!(
                    "Dropping: PTY session with PID {} closed via Drop.",
                    pty.pid
                );
            } else {
