...
impl PtyProcess {
    fn new() -> io::Result<Self> {
        ...
        #[cfg(target_os = "linux")]
        {
            unimplemented!("Linux PTY creation not implemented")
        }
        #[cfg(target_os = "macos")]
        {
                ...
                match command {
                    PtyCommand::Write(data) => {
                        ...
                        if bytes_written < 0 {
                            ...
                            info!("Wrote {} bytes to PTY.", bytes_written); // Include bytes_written in the log
                        }
                    }
...
impl PtyHandle {
    ...
    pub async fn new() -> Result<Self> {
        ...
        let handle = match timeout(creation_timeout, handle_future).await {
            Ok(result) => result.map_err(|e| Error::new(Status::GenericFailure, format!("Failed to create PtyHandle: {:?}", e)))?,
            Err(e) => {
                return Err(Error::new(Status::GenericFailure, format!("PtyHandle creation timed out: {:?}", e)));
            }
        };
    ...
    pub async fn read(&self) -> Result<String> {
        ...
        match timeout(read_timeout, read_future).await {
            ...
            Err(e) => {
                ...
                Err(Error::new(Status::GenericFailure, format!("Read operation timed out: {}", e)))
            }
        }
    }
    ...
    pub async fn close(&self) -> Result<()> {
        ...
        let close_future = task::spawn_blocking(move || -> std::result::Result<(), String> {
        ...
        let close_result = timeout(graceful_timeout, close_future).await;
    ...
    async fn force_kill(&self, force_timeout: Duration) -> Result<()> {
        ...
        let force_kill_future = task::spawn_blocking(move || -> std::result::Result<(), String> {
            let binding = weak_binding.upgrade().ok_or_else(|| "Failed to upgrade weak reference".to_string())?;
            let mut pty_option = binding.lock();
        ...
        let force_kill_result = timeout(force_timeout, force_kill_future).await;
                ...
                if let Some(pty) = pty_option.take() {
                    ...
                    if kill_ret != 0 {
                        ...
                        if wait_ret == -1 {
                            ...
                            info!("Dropping: Process {} terminated gracefully.", pty.pid); // Include pty.pid in the log
                        }
                    }
