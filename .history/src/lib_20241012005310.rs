...
impl PtyHandle {
    ...
    pub async fn read(&self) -> Result<String> {
        ...
        let read_future = task::spawn_blocking(move || -> Result<String, napi::Error> {
            let master_fd_opt = {
        ...
        let read_result = timeout(read_timeout, read_future)
            .await
            .map_err(|_| Error::new(
                Status::GenericFailure,
        ...
        let data = match read_result {
            Ok(inner_result) => inner_result.map_err(|e| e)?,
            Err(join_err) => {
                error!("Read task failed: {}", join_err);
                return Err(Error::new(
                    Status::GenericFailure,
                    format!("Read task failed: {}", join_err),
                ));
            }
        };
    ...
    pub async fn write(&self, data: String) -> Result<()> {
        ...
        let write_future = task::spawn_blocking(move || -> Result<(), napi::Error> {
            let master_fd_opt = {
                let pty_guard = pty_lock.lock().map_err(|e| {
                    error!("Failed to acquire PTY lock: {}", e);
                    Error::new(Status::GenericFailure, "Failed to acquire PTY lock")
        ...
        let write_result = timeout(write_timeout, write_future)
            .await
            .map_err(|_| Error::new(
                Status::GenericFailure,
        ...
        match write_result {
            Ok(inner_result) => inner_result.map_err(|e| e)?,
            Err(join_err) => {
                error!("Write task failed: {}", join_err);
                return Err(Error::new(
                    Status::GenericFailure,
    ...
    pub async fn resize(&self, cols: u16, rows: u16) -> Result<()> {
        ...
        let resize_future = task::spawn_blocking(move || -> Result<(), napi::Error> {
        ...
        let resize_result = timeout(resize_timeout, resize_future)
            .await
            .map_err(|_| Error::new(
                Status::GenericFailure,
        ...
        match resize_result {
            Ok(inner_result) => inner_result.map_err(|e| e)?,
            Err(join_err) => {
                error!("Resize task failed: {}", join_err);
                return Err(Error::new(
                    Status::GenericFailure,
    ...
    pub async fn close(&self) -> Result<()> {
        ...
        let close_future = task::spawn_blocking(move || -> Result<(), napi::Error> {
        ...
        let close_result = timeout(close_timeout, close_future)
            .await
            .map_err(|_| Error::new(
                Status::GenericFailure,
        ...
        match close_result {
            Ok(inner_result) => inner_result.map_err(|e| e)?,
            Err(join_err) => {
                error!("Close task failed: {}", join_err);
                return Err(Error::new(
                    Status::GenericFailure,
                    format!("Close task failed: {}", join_err),
                ));
            }
        };
