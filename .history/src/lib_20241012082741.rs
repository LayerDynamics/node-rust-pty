...
impl PtyHandle {
    ...
    pub async fn read(&self) -> Result<String> {
        ...
        let result = timeout(read_timeout, task::spawn_blocking(move || -> Result<String, napi::Error> {
        ...
        match result {
            ...
            Ok(Err(e)) => {
                error!("Error in reading from PTY: {}", e);
                self.send_result(PtyResult::Failure(format!("Read operation failed: {}", e)));
                Err(e)
            }
            Err(_) => {
                error!("Read operation timed out after {:?}", read_timeout);
                self.send_result(PtyResult::Failure("Read operation timed out".into()));
                Err(Error::new(Status::GenericFailure, "Read operation timed out"))
            }
        }
    }
