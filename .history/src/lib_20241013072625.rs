...
impl PtyHandle {
    ...
    pub async fn new() -> Result<Self> {
        ...
        let pty_handle_result: std::result::Result<PtyHandle, std::io::Error> = timeout(creation_timeout, task::spawn_blocking(|| {
            ...
            Ok::<PtyHandle, io::Error>(PtyHandle {
        ...
        .await.map_err(|e| napi::Error::from_reason(format!("Timeout error: {}", e)))?
              .map_err(|e| napi::Error::from_reason(format!("Join error: {}", e)))?;

        let pty_handle = pty_handle_result.map_err(|e| {
            napi::Error::from_reason(format!("Failed to create PtyHandle: {}", e))
        })?;
