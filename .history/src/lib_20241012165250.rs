...
mod platform {
    ...
    impl PtyProcess {
        pub fn new() -> io::Result<Self> {
            ...
            let ret = unsafe { openpty(&mut master_fd, &mut slave_fd, ptr::null_mut(), &mut ws, ptr::null_mut()) };
            if ret != 0 {
                return Err(io::Error::last_os_error());
            }
...
// Structure representing the PTY process is already handled
    ...
    async fn force_kill(&self, force_timeout: Duration) -> Result<()> {
        ...
        let force_kill_result = timeout(force_timeout, task::spawn_blocking(move || -> std::result::Result<(), String> {
            ...
            if let Some(pty) = pty_option.take() {
                ...
                if let Err(e) = pty.kill_process(SIGKILL) {
    ...
    pub async fn read(&self) -> Result<String> {
        ...
        let read_future = task::spawn_blocking(move || -> std::result::Result<String, napi::Error> {
            ...
            READ_BUFFER.with(|buffer_cell| {
                ...
                let bytes_read = pty_process.read_data(&mut buffer)?;
...
impl PtyHandle {
    // Implement `force_kill` inside `impl PtyHandle`
}
