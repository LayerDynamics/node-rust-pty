...
pub async fn close(&self) -> Result<()> {
    ...
    let close_result = timeout(graceful_timeout, task::spawn_blocking(move || -> Result<()> {
        let mut pty_option = pty_lock.lock().map_err(|e| {
            error!("Failed to acquire PTY lock: {}", e);
            Error::new(Status::GenericFailure, format!("Failed to acquire PTY lock: {}", e))
        })?;
...
async fn force_kill(&self, force_timeout: Duration) -> Result<()> {
    ...
    let force_kill_result = timeout(force_timeout, task::spawn_blocking(move || -> Result<()> {
        let mut pty_option = pty_lock.lock().map_err(|e| {
            error!("Failed to acquire PTY lock: {}", e);
            Error::new(Status::GenericFailure, format!("Failed to acquire PTY lock: {}", e))
        })?;
