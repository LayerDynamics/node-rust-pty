...
/// Define the PtyHandle struct and implement FinalizationRegistry-like behavior
#[napi]
#[derive(Clone)]
pub struct PtyHandle {
    // Shared state wrapped in Arc and Mutex for thread safety
    pty: Arc<Mutex<Option<PtyProcess>>>,
    // Channels for command communication
    command_sender: Sender<PtyCommand>,
    command_receiver: Receiver<PtyCommand>,
    result_sender: Sender<PtyResult>,
    result_receiver: Receiver<PtyResult>,
...
impl PtyHandle {
    ...
    // This block can be used to add additional methods if needed
}

/// The following enums are used for internal command and result handling
#[derive(Debug)]
enum PtyCommand {
    Write(Bytes),
    Resize(u16, u16),
