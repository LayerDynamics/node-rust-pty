...
impl PtyHandle {
    ...
    pub async fn write(&self, data: String) -> Result<()> {
        ...
        let write_future = timeout(write_timeout, task::spawn_blocking(move || -> Result<()> {
            let master_fd_opt = {
                let pty_guard = pty_lock.lock().map_err(|e| {
                    error!("Failed to acquire PTY lock: {}", e);
                    Error::new(Status::GenericFailure, "Failed to acquire PTY lock")
        ...
        match write_future.await {
            Ok(inner_result) => inner_result, // Result<(), napi::Error>
            Err(_) => {
    ...
    pub async fn resize(&self, cols: u16, rows: u16) -> Result<()> {
        ...
        let resize_future = timeout(resize_timeout, task::spawn_blocking(move || -> Result<()> {
    ...
    pub async fn close(&self) -> Result<()> {
        ...
        let close_future = timeout(close_timeout, task::spawn_blocking(move || -> Result<()> {
