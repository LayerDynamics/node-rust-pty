...
impl PtyHandle {
    ...
    pub async fn read(&self) -> Result<String> {
        ...
        // Proper handling of the result, ensuring that `data` is unwrapped correctly
      match result {
    Ok(Ok(data)) => {
    ...
    Ok(Err(err)) => {
        // Convert `JoinError` into a `napi::Error`
        error!("Error during read operation: {:?}", err);
        Err(Error::new(Status::GenericFailure, format!("Join error: {:?}", err)))
    },
    Err(elapsed_err) => {
