...
impl PtyHandle {
    ...
    pub async fn new() -> Result<Self> {
        ...
        // Await the handle_future with timeout
        let pty_handle = handle_future.await.map_err(|e| {
...
impl PtyHandle {
    ...
    #[napi]
    pub async fn read(&self) -> Result<String> {
        PtyHandle::read(self).await
    }

    /// Asynchronously writes data to the PTY.
    #[napi]
    pub async fn write(&self, data: String) -> Result<()> {
        PtyHandle::write(self, data).await
    }

    /// Asynchronously resizes the PTY window.
    #[napi]
    pub async fn resize(&self, cols: u16, rows: u16) -> Result<()> {
        PtyHandle::resize(self, cols, rows).await
    }

    /// Asynchronously closes the PTY gracefully.
    #[napi]
    pub async fn close(&self) -> Result<()> {
        PtyHandle::close(self).await
    }
}
