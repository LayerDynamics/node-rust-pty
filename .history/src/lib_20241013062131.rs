...
//! # node-rust-pty
//!
//! A Rust-based PTY (Pseudo-Terminal) handler for Node.js using `napi-rs`. This module allows
//! the creation, management, and interaction with PTY processes from JavaScript.
//!
//! ## Features
//!
//! - **Cross-Platform Support:** Currently supports Linux and macOS. Other platforms are marked as unsupported.
//! - **Asynchronous Operations:** Utilizes Tokio for asynchronous tasks, ensuring non-blocking interactions.
//! - **Command Handling:** Supports writing data to the PTY and resizing the PTY window.
//! - **Process Management:** Ability to gracefully terminate or forcefully kill the PTY child process.
//!
//! ## Limitations of `napi-rs` Integration
//!
//! While `napi-rs` provides a robust bridge between Rust and Node.js, there are inherent limitations and considerations:
//!
//! - **Type System Differences:**
//!   - Rust's `Result` type is converted to exceptions in JavaScript.
//!   - `Option` types in Rust translate to `undefined | T` in TypeScript.
//!   - Auto-generated TypeScript definitions may not perfectly align with Rust's type system.
//!
//! - **Performance Considerations:**
//!   - Native addon function calls can introduce performance overhead compared to pure JavaScript implementations.
//!   - Serialization and deserialization of complex types between Rust and JavaScript can impact performance.
//!
//! - **Debugging Challenges:**
//!   - Release binaries often lack debug symbols, complicating the debugging of runtime crashes or panics.
//!   - Error stacks may not be fully descriptive due to the absence of debug symbols.
//!
//! - **Distribution and Trust Issues:**
//!   - Pre-compiled binaries cannot be easily audited, potentially raising security concerns.
//!   - NPM's Provenance feature helps mitigate some trust issues by providing signed packages.
//!
//! - **API Limitations:**
//!   - Limited support for certain JavaScript features like async generators or Node.js streams.
//!   - Some abstractions may not align seamlessly with Rust's philosophy, though ongoing development aims to address these.
//!
//! Despite these limitations, `napi-rs` continues to evolve, with improvements targeting performance, debugging capabilities, and API refinements.
...
/// Thread-local buffer for reading from PTY
thread_local! {
...
/// Initializes logging. Ensures it's only initialized once.
...
impl PtyHandle {
    /// Handles incoming commands by spawning a Tokio task.
    ...
    /// Sends results back through the result channel.
    ...
    /// Listens for results and logs them accordingly.
...
impl PtyHandle {
    ...
    pub async fn new() -> Result<Self> {
        ...
        // Spawn the blocking task to initialize PtyProcess and channels
        let pty_handle_result = timeout(creation_timeout, task::spawn_blocking(|| {
        ...
        let pty_handle = match pty_handle_result {
            Ok(handle_result) => handle_result.map_err(|e| {
                Error::new(Status::GenericFailure, format!("Failed to create PtyHandle: {:?}", e))
            })?,
            Err(_) => {
                return Err(Error::new(
                    Status::GenericFailure,
                    "PtyHandle creation timed out",
                ));
            }
        };

        // At this point, pty_handle is of type PtyHandle
    ...
    pub async fn read(&self) -> Result<String> {
        ...
        match timeout(read_timeout, read_future).await {
            Ok(inner_result) => inner_result.map_err(|e| {
                Error::new(Status::GenericFailure, format!("Read operation failed: {:?}", e))
            })?,
            Err(_) => {
    ...
    pub async fn close(&self) -> Result<()> {
        ...
        match close_result {
            ...
            Err(_) => {
                warn!("Graceful shutdown timed out. Initiating force kill.");
                self.send_result(PtyResult::Failure("Graceful shutdown timed out.".into()));
