...
impl PtyHandle {
    ...
    pub async fn resize(&self, cols: u16, rows: u16) -> Result<()> {
        ...
        let _result = timeout(resize_timeout, task::spawn_blocking(move || -> Result<()> {
            ...
            let master_fd_opt = {
                let pty_guard = pty_lock.lockIt looks like part of my response was cut off. Here is the continuation and completion of the full implementation of your file:
    ...
    pub async fn close(&self) -> Result<()> {
        ...
        let close_result = timeout(graceful_timeout, task::spawn_blocking(move || -> std::result::Result<(), String> {
            let mut pty_option = pty_lock.lock().map_err(|e| {
                error!("Failed to acquire PTY lock: {}", e);
                format!("Failed to acquire PTY lock: {}", e)
            })?;
