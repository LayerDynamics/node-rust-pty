...
pub async fn close(&self) -> Result<()> {
    ...
    // Set a reasonable timeout duration for the close operation (e.g., 10 seconds)
    ...
    let result = timeout(close_timeout, task::spawn_blocking(move || -> Result<()> {
        ...
        if let Some(pty) = pty_option.take() {
            ...
            // If process is still alive, force kill with SIGKILL
    ...
    .await??;  // This correctly unwraps both the timeout result and the inner spawn result

    // Handle timeout and force a SIGKILL if necessary
    Ok(result)
}
